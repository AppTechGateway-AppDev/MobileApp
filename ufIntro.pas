...
//    property OnSubmit: TProc read FOnSubmit write FOnSubmit;
    ...
    property OnSignInBtnClick: TProc read FOnSignInBtnClick write FOnSignInBtnClick;
...
procedure TfraIntro.Rectangle1Click(Sender: TObject);
begin
  AnimateTrans(TAnimateRule.arOut);  // animate the main tab
//  TTabOfFrame(Owner).CloseTab(false);
  if Assigned(FOnSignInBtnClick^) then    // show user regis tab
    FOnSignInBtnClick^();
end;
...
//  FFrameMain.FOnSubmit := @OnSubmit;
  ...
  FFrameMain.FOnSignInBtnClick := @OnSignInBtnClick;
end;
