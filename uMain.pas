...
//    procedure Button1Click(Sender: TObject);
  ...
  private
    ...
    procedure ShowUserRegis(ACallerTab: TTabItem);
    procedure ShowIntro;
...
begin
  ...
  begin
    ...
    lTabItem.OnRegisBtnClick :=
      procedure
      begin
        ShowUserRegis(lTabItem);      // go back to the intro's last page.
        //lTabItem.ShowTab(TabControl1.ActiveTab);
      end;
    lTabItem.OnSignInBtnClick :=
      procedure
      var
        lTabItem: ufSignIn.TTabOfFrame;
      begin
...
begin
  ...
  begin
    ...
    lTabItem.OnSubmit :=
      procedure()
      begin
        // this portin may be recoded to check whether Submit has succeeded or failed.
...
begin
  mvMain.HideMaster; // .Visible := false;
  ShowUserRegis(TabControl1.ActiveTab);
...
procedure TfMain.ShowUserRegis(ACallerTab: TTabItem);
var
  lTabItem: ufUserRegis.TTabOfFrame;
begin
//  mvMain.HideMaster; // .Visible := false;
  lTabItem := ufUserRegis.TTabOfFrame(CreateUserRegis);
  //lTabItem.ShowTab(TabControl1.ActiveTab);  // this will bring back thje main menu
  lTabItem.ShowTab(ACallerTab);
